...
export const fetchPerson =(id) =>async (dispatch)=>{
  ...
  try {
    ...
    const tv_credits = await axios.get(process.env.GET_DETAIL_PERSON + id + '/tv_credits?api_key=' + process.env.API_KEY)
      .then(response => response.data);

    const person_image = await axios.get(process.env.GET_DETAIL_PERSON + id + '/images?api_key=' + process.env.API_KEY)
      .then(response => response.data);

    dispatch(personFetchFulfilled(person, know_for, externalIds, tagged_images, tv_credits, person_image));
  }
  catch (err){
